...
- `scripts/check-site-config.ts` — 离线一致性校验（七条断言：`--target` 合法、wrangler routes ⊆ 配置、工作流无硬编码域名、CORS 覆盖全部目标、基线文件名与 `r2.baselineObject` 一致、版本来源唯一、wrangler vars 与配置一致）。`pnpm check:config` 调用它，pre-commit 与 CI 都跑。
