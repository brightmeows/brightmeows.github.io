...
- `scripts/check-site-config.ts` — 离线一致性校验（六条断言：`--target` 合法、wrangler routes ⊆ 配置、工作流无硬编码域名、CORS 覆盖全部目标、基线文件名与 `r2.baselineObject` 一致、版本来源唯一）。`pnpm check:config` 调用它，pre-commit 与 CI 都跑。
...
- **`worker/` 按“路由、处理、数据”分三层** — `index.ts` 是入口（`/bms/table/mirror/*` 动态路由与 `/api/internal/*` 的分派）；`api.ts` 与 `admin.ts` 是 `/api/*` 与 `/api/admin/*` 的**路由表**，只做分派，站长身份与同源校验留在 `admin.ts`；处理逻辑在 `handlers/`（`account` 登录态、`preview` 预览与 header 提取、`tables` 增删与回收站、`governance` 治理动作）；数据读写集中在 `store.ts`；`auth.ts`、`dispatch.ts`、`backup.ts`、`schema.ts`/`schema-sql.ts`、`manifest.ts` 各管一块。新增接口时按这个分层放代码，不要把校验或业务写回路由文件。
