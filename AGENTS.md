...
- **旧配置迁移已完成（2026-09）** — 存档的旧 toml（`scripts/pipeline/fixtures/legacy-table.toml`）已通过一次性脚本转成 R2 用户层对象（added 10、禁用 1、替换 2、元数据覆盖 10、空名单），脚本与 `migrate-config` 工作流已在完成后删除。fixture 与 `legacy-config.ts` 保留：对拍（`parity`）仍靠它们驱动旧二进制（它固定找 `config/table.toml`，`prepareBaseline` 会拷过去）。需要重跑迁移时从 git 历史取回脚本，脚本在线上 `user/added.json` 非空时会中止（除非 `--force`）。
