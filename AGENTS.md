...
- `node scripts/fetch-tables.ts` — 本地跑数据管线（活跃表集合来自 R2 基线叠加用户层，写 `tables/`、`indexes/`、`warnings.log`；需要网络，产物已 gitignore）
- `node scripts/fetch-table-once.ts --url=<表源> --request-id=<id> --out-dir=./out` — 单表抓取（复用管线抓取与命名逻辑），产物供 `fetch-table` 工作流上传到 R2
...
- **数据管线只提交清单快照，且只在列表变动时提交** — `update-tables` 先抓取并双向同步 R2，然后跑 `scripts/sync-mirror-manifest.ts`：比对 R2 清单与 `src/lib/mirror/table-manifest.json` 的投影（`dir_name` 集合 + 展示字段，不含 `date`/`comment`/`state`），**仅列表变动**时提交并以 `MIRROR_SYNC_DEPLOY_KEY`（仓库写权限 deploy key）推送，从而连锁触发 CI/Deploy/Mirror 重建三个目标。表内容更新不产生提交，因此不触发部署。快照位于 `src/lib/mirror/` 下，不命中本工作流的触发路径，不会自触发。用 deploy key 是因为 `GITHUB_TOKEN` 的 push 不会触发后续工作流。
...
- **清单在主站读取侧合成，用户层存于 R2 `user/` 前缀** — Worker（`worker/index.ts`）把管线清单（R2 `tables/tables.json`，含快照兑底）与用户层（添加、删除、禁用、替换、授权、元数据覆盖）合并后响应：`protected` 标记随条目输出，单表页校验也用合成清单。用户层经 R2 binding（`MIRROR_BUCKET`）读取，同 isolate 内 60 秒内存缓存；用户层不可用时退化为纯管线清单——清单可用优先于增删可见。**静态宿主不再读仓库快照**，改为拉主站 `tables.json`（见 Commands 的生成器）：合成逻辑只存在于 Worker 一处，代价是静态构建需要网络。桶名因此在 `wrangler.jsonc` 的绑定里入库（凭据仍不入库）。
- **单表抓取是独立工作流（`fetch-table`）** — 用户提交添加后由 Worker 触发，也可手动 dispatch 演练：复用管线的 `fetchTable` 与命名逻辑（`scripts/fetch-table-once.ts`），把 `tables/<dir_name>/` 数据、`user/fetched/<id>.json` 与 `user/status/<id>.json` 写到 `out/` 后经 rclone 上传；**失败只写 failed 状态、不产出数据目录**（半成品目录会被管线当作基线）。抓取脚本退出码恒为 0（除参数错误），结果由 `out/result.json` 表达——状态文件必须上传，前端据此结束轮询。成功后触发一次 `deploy`，需要 `WORKFLOW_DISPATCH_TOKEN`（fine-grained PAT，仅 actions:write）。输入经 env 传入、不直接插值进 shell 命令。
- **写接口在 `/api/*`，登录走 GitHub OAuth** — 会话是 HMAC 签名 cookie（30 天，HttpOnly/Secure/SameSite=Lax）；写操作另做同源 Origin 校验，并消耗每日配额（每账号 10 次，按 UTC 日）。删除先写黑名单（可见性优先）再移入 `trash/<时间戳>/<dir_name>/`，作者可在 30 天内自助恢复（`TRASH_RETENTION_DAYS` 与工作流清理窗口同源）；索引对象用 R2 条件写（ETag）乐观锁重试；审计逐条独立成对象（`user/audit/`）；增删后触发部署（10 分钟节流）。清单合成缓存（同 isolate 60 秒）在写操作后主动失效。Worker secrets：`GITHUB_OAUTH_CLIENT_ID`、`GITHUB_OAUTH_CLIENT_SECRET`、`SESSION_SECRET`、`GITHUB_DISPATCH_TOKEN`（fine-grained PAT，仅 actions:write）。
...
- **旧配置迁移是一次性工作流（`migrate-config`）** — 把存档的旧 `config/table.toml`（`scripts/pipeline/fixtures/legacy-table.toml`）转成 R2 用户层对象（added/fetched/disabled/replace/meta 与空名单），经 rclone 上传。脚本在线上 `user/added.json` 非空时中止（避免覆盖真实用户数据），`force` 输入可跳过；迁移完成后脚本与工作流可删除。对拍（`parity`）同样读这份 fixture 驱动旧二进制（它固定找 `config/table.toml`，`prepareBaseline` 会拷过去）。
