...
- **`static/bms/table/mirror/tables.json` 为唯一构建依赖** — 仓库内仅含空 `[]`，CI（`.forgejo/workflows/deploy.yml`）每 6 小时从 R2 拉取替换为真实数据。表数据（`header.json`/`data.json`）和搜索索引全在客户端运行时从 R2 拉取，构建时不依赖。
...
- **BMS 数据源分流** — `static/bms/table/` 下 `self-sp/`、`self-dp/`、`satellite-skill-analyzer-3rd-preview/`、`starlight-preview/` 在 git 中；`mirror/` 仅含 `tables.json`（由 CI 从 R2 拉取），表数据（`header.json`/`data.json`）和搜索索引全从 R2 客户端拉取。修改数据入口时区分来源。
